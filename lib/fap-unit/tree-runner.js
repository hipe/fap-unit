 ...
 * each of whose elements is null if the `list` item at the same index
 * has no children, and if not null is an array of the indexes of the
 * children the item has, except for the last item of the map which
...
function _buildChildrenMap(list) {
  ...
  for (var idx = 0, num = list.length - 1; idx < num; idx++) {
...
function _renderTree(list, chx, childs, out, pre, mid, fin) {
  if (!pre) pre = '';
  if (!mid) mid = '';
  if (!fin) fin = '';
  for (var i = 0, last = childs.length - 1; i <= last; i++) {
    ...
    if (chx[idx]) {
      var chPre = pre + (i==last ? ' ' : '| ');
      var chMid = "|-";
      var chFin = "'-";
      _renderTree(list, chx, chx[idx], out, chPre, chMid, chFin);
    }
  }
}
