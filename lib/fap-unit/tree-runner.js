...
TreeRunner.prototype = {
  ...
  run : function(request, out) {
    ...
    var list = this._buildFileList(usePaths);
    //_inspectList(list);
    var tree = new TestTree(list);
    if ((request.tree || request['tree-files']) &&
      ! this._displayTree(tree, request.tree)) return null; // exit ok

    // foreach test case in the tre
  },
  ...
  _buildFileList : function(paths) {
    var list = [], me = this;
    fu.suppressRun(function(){
      for (i = 0; i < paths.length; i++) {
        me._flatten(paths[i], paths[i], list, null, 1);
      }
    });
    return list;
  },
  _displayTree : function(tree, showTests) {
    tree.render(this.__out, showTests); // --tree means show test names
    return false; // never keep going, for now
  },
  _flatten : function(path, basename, list, pix, addTests) {
    var full = Path.resolve(path), mod, i, myIdx;
    ...
    if (fs.statSync(full).isDirectory()) {
      ...
      for (i = 0; i < paths.length; i++) {
        this._flatten(
          Path.join(path, paths[i]), paths[i], list, myIdx, addTests
        );
      }
    ...
    _renderTree(this.__list, this.__children, chx,
      ...
      function(prefix, node){
        ...
        switch (node.nodeType) {
          ...
          case 'test' :
            if (showTests) {
              title = color('"' + node.testName +'"', 'green');
            } else {
              doNode = false;
            }
            break;
        ...
        if (doNode) rows.push([null, basename, title]);
      }
    );
    tableize(rows, [{align:'left'},{align:'left', padLeft:' '}], out);
  }
};
