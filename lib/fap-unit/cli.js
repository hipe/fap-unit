...
exports.command = optparse.build(function(o) {
  o.desc('Generates and runs fap-unit tests.');
  ...
  o.on('run', "Run the test module(s) in the file(s) or folder(s)",
    ...
    p.on('-f FMT', '--format=FORMAT',
                'Output test run info in the given FORMAT',
                '({enum}) (default: {default})',
                 ['tree', 'orig'], { 'default' : 'orig' });

    p.on('-e', '--tree[=WHICH]',
              'Just show the tree of matching files [and cases [and tests]]',
              'and exit. ({enum}) (default: {enumDefault})',
               ['file', 'case', 'test'], { enumDefault : 'file' });
    ...
    p.execute(function() {
      return require('./tree-runner').run(this.request.values, this.out);
    });
  });
});
 ...
 * subcommands and folders.  subcommand subcommand subcommand.
 */
...
exports.command.handleMissingRequiredPositional = function(param, ctx) {
  ...
  if (useDir) {
    ...
    // the `path` element of the request (an array) (@spot1) will have been
