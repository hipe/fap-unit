...
Runner.prototype = {
  ...
  _announceSummary : function() {
    ...
    if (0==this.num.tests && this.__filters.any() && this.__noticeOnEmptyMatch)
      this._puts(
  ...
  _parseCommandLineOptions : function() {
    ...
    var parser = optparse.build(require('./fap-unit/cli').common), me = this;
    ...
    if (req.values.name &&
      ! this.__filters.addTestFilters(req.values.name, function(e) {
        me._puts(e);
      })) return false;
    if (req.values.testcase &&
      !this.__filters.addTestCaseFilters(req.values.testcase, function(e) {
        me._puts(e);
      })) return false;
