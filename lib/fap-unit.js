...
DefaultFormatter.prototype = {
  beforeRunAll   : function() { },
  testCaseLoaded : function(tc, asChild) {
    asChild || this._puts("Loaded case "+(tc.name || '[no name]'));
    // @todo: in ruby test-unit this says "loaded suite". what does it mean?
  },
  testCaseStarted : function(asChild) {
    asChild || this._puts("Started");
  },
  ...
  /**
   * @return the first line of the backtrace that is not from this file!
   */
  _relevantStackSlice : function(str) {
    var bt = require('./fap-unit/util').backtrace(str), item;
    bt.next(); // top item, should be same as exception.toString();
    while ((item = bt.next()) && item.path && __filename == item.path); // !
...
/**
 * Wrap each of the 9 (at the time of this writing) assertions that are
 * available in assert.js but hook into our API to increment counts
 * and call callbacks, etc.
 *
 * Experimentally we may add some new assertions of our own.
 *
 */
...
// #metaprogramming!
var _these = ['ok', 'equal', 'notEqual', 'deepEqual', 'notDeepEqual',
  'strictEqual', 'notStrictEqual', 'throws', 'doesNotThrow'];
for (var i = 0; i < _these.length; i++) {
  (function(name) {
    Assertions.prototype[name] = function() {
      ++ this.num.asserts;
      try {
        assert[name].apply(assert, arguments);
        this._okNotify();
      } catch (e) {
        'AssertionError' == e.name && this._failNotify();
        throw e;
      }
    };
  })(_these[i]);
...
function inspectString(str, left, right) {
  if (!(/\n/).test(str)) return '"' + str + '"';
  if (undefined == left) left = '    ....';
  if (undefined == right) right = '....';
  var derp = str.replace(/^(.*)$/gm, function(inner){
...
function enhanceMatchFailMessage(actual, regexp, message) {
  var useNeed = color(inspectString(regexp.toString(), '  -', '$'), 'red');
  var useHave = color(inspectString(actual, '  +', '$'), 'green');
  parts = [];
  parts.push(useNeed);
...
Runner.prototype = {
  ...
  _beforeRunCase : function(asChild) {
    ...
    this.__formatter.testCaseLoaded(this.testcase, asChild);
    this.__formatter.testCaseStarted(asChild);
  },
  _afterRunCase : function(asChild) {
    this.__formatter.testCaseFinished(asChild);
  },
...
FapCase.proto = {
  ...
  childCases : function() { return this.__children; },
  /**
...
/**
 * Make all assertion methods be available on the TestCase.
 * experimental! be very careful here! #metaprogramming in js!?
 */
var prop;
for (prop in Assertions.prototype) {
  if ('_' == prop.substr(0,1)) continue;
  (function(prop) {
    FapCase.proto[prop] = function() {
      return this.assert[prop].apply(this.assert, arguments);
    };
  })(prop);
}
